...
//
// - slug-do-produto é opcional (padrão: pilates-somatico)
...
// e têm espaço, então não colidem). Default: pilates-somatico.
const slug = resto.find((a) => /^[a-z0-9][a-z0-9-]*$/.test(a)) ?? 'pilates-somatico'
...
async function main() {
  ...
  if (!product) {
    console.error(`✗ Produto "${slug}" não encontrado. Confira o slug (ex.: pilates-somatico).`)
    process.exit(1)
  }
