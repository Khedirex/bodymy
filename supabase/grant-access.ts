...
//
// - slug-do-produto é opcional (padrão: caminhada-japonesa)
...
// e têm espaço, então não colidem). Default: caminhada-japonesa.
const slug = resto.find((a) => /^[a-z0-9][a-z0-9-]*$/.test(a)) ?? 'caminhada-japonesa'
...
async function main() {
  ...
  if (!product) {
    console.error(`✗ Produto "${slug}" não encontrado. Confira o slug (ex.: caminhada-japonesa).`)
    process.exit(1)
  }
