  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-slate-500">
        <Link href={`/admin/exercicios?c=${ex.circuito}`} className="hover:underline">← Circuito ({ex.circuito})</Link>
        <span>/</span>
        <span className="text-slate-800">{ex.nome}</span>
      </div>
      <p className="text-sm text-slate-500">
        Día {ex.dia_do_ciclo} del ciclo · posición {ex.ordem_no_dia} · ejercicio{' '}
        {ex.ordem_no_circuito}
      </p>
