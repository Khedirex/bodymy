...
const TIPOS: { v: ProductTipo; label: string }[] = [
  ...
  { v: 'dieta_premium', label: 'Menú premium' },
  { v: 'bundle', label: 'Combo (paquete)' },
  { v: 'extra', label: 'Extra' },
]
...
export function ProductForm({ product }: { product: Product | null }) {
  ...
  async function salvar() {
    ...
    try {
      ...
      if (!res.ok) {
        setMsg({ tipo: 'erro', texto: `Falló: ${data.error ?? 'error'}` })
        return
      }
      setMsg({ tipo: 'ok', texto: data.mensagem ?? 'Guardado.' })
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-4">
        ...
        <fieldset className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
          <legend className="px-1 text-xs font-semibold uppercase text-slate-500">Datos</legend>
          <label className="block text-sm"><span className="text-slate-600">Nombre</span><input className={inputCls} value={f.nome} onChange={(e) => set('nome', e.target.value)} /></label>
          <label className="block text-sm"><span className="text-slate-600">Slug</span><input className={inputCls} value={f.slug} onChange={(e) => set('slug', e.target.value)} placeholder="solo-minusculas-y-guiones" /></label>
          <label className="block text-sm"><span className="text-slate-600">Descripción</span><textarea className={inputCls} rows={2} value={f.descricao} onChange={(e) => set('descricao', e.target.value)} /></label>
          <div className="flex gap-3">
            ...
            <label className="block flex-1 text-sm"><span className="text-slate-600">Precio (visualización)</span><input className={inputCls} value={f.preco_exibicao} onChange={(e) => set('preco_exibicao', e.target.value)} placeholder="R$ 37,00" /></label>
          </div>
          <label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={f.ativo} onChange={(e) => set('ativo', e.target.checked)} /> Activo</label>
        </fieldset>
        ...
        <fieldset className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
          <legend className="px-1 text-xs font-semibold uppercase text-slate-500">Página de venta (/oferta)</legend>
          ...
          <label className="block text-sm"><span className="text-slate-600">Imagen (URL)</span><input className={inputCls} value={f.imagem_url} onChange={(e) => set('imagem_url', e.target.value)} /></label>
          <label className="block text-sm"><span className="text-slate-600">Bullets (uno por línea)</span><textarea className={inputCls} rows={4} value={f.bullets} onChange={(e) => set('bullets', e.target.value)} /></label>
          <label className="block text-sm"><span className="text-slate-600">Texto del botón</span><input className={inputCls} value={f.cta_label} onChange={(e) => set('cta_label', e.target.value)} placeholder="QUIERO ESTE PROGRAMA" /></label>
        </fieldset>
        ...
        <button onClick={salvar} disabled={salvando} className="w-full rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-50">
          {salvando ? 'Guardando…' : f.id ? 'Guardar producto' : 'Crear producto'}
        </button>
      </div>
